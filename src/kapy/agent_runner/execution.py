...
Only successful fenced transactions advance them. The per-SDK-run capability
owns preparation/node observations, never a lease or a shared Agent's deps.
"""
...
class ExecutionState:
    ...
    def __init__(
        ...
        *,
        ...
        self.session_id, self.lock_token = session_id, lock_token
    ...
    async def consume_cancel(self, consume: ConsumeCancel) -> bool:
        ...
        async with self.session_factory.begin() as db:
            await AgentRepository(db).lock_owned(self.session_id, self.lock_token)
    ...
    async def checkpoint(self, next_step: NextStep, messages: Sequence[ModelMessage]) -> None:
        ...
        async with self.session_factory.begin() as db:
            ...
            await repo.lock_owned(self.session_id, self.lock_token)
    ...
    async def accept_inputs(
        ...
        *,
        ...
        async with state.session_factory.begin() as db:
            repo = AgentRepository(db)
            await repo.lock_owned(state.session_id, state.lock_token)

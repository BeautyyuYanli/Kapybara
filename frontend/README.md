...
close-session responses and per-session context plugin selection. The session form
accepts a registered name on creation and JSON config; edits keep the name fixed.
The host reference-round setting is separate from plugin config. No provisional schema or handwritten API DTOs are
